...
public class ToDoList
{
     ...
     * 12. прописать более плавное перемещенение верхней камеры
     * 13. переделать getCurrentPlayer в GameCanvas так как не подходит для мультиплеера (всегда возвращает только первого игрока) (в Gamecontroller тоже есть currentplayer)
     * 14. сделать запись сохранения количества сделанных ходов
     * 15. дописаны ли методы в mousecontroller для зданий и игроков?
     * 16. до какого отчаяния мы дошли, что сохраняем канву в плеере? переписать, чтоб канва хранилась в дбворке
     * 17. юнька ругается, что в mousecontroller много объявленных, но не используемых переменных (смотри предупреждения)
     * 18. реализовать поворот моделек
     * 19. в плеере в апдейте закидываются данные в канву. завязано на айдишнике,  не грохнет ли в мультиплеере?
     * 20. а где вообще можно посмотреть какие есть монополии и какие улицы туда входят? где доп инфу о монополии посмотреть, она же есть в бд
     * 21. разобраться с LOD
     * 22. сделать запись выпавшего числа ходов  на кубиках
     */
