...
public class Ways
{
	...
	public void createWays(StreetPath[] streetPaths)
	{
		...
		foreach (StreetPath path in streetPaths)
		{
			...
			{
				if ((path.isBridge && path.start.Equals(streetPaths[i].start)) || path.end.Equals(streetPaths[i].start) ||(streetPaths[i].isBridge && path.end.Equals(streetPaths[i].end)))
				{
		...
		for (int i = 1; i < streetPaths.Length; i++)
		{
			...
			for (int j = 1; j < streetPaths.Length; j++)
			{
				...
				{
					if ((_queues[j, k] == null && _queues[j, i] != null && _queues[i, k] != null) || (_queues[j, k] != null && _queues[j, i] != null && _queues[i, k] != null &&
